/*!/*:
@target MZ
@plugindesc  角色图像扩展系统v1.1.3
@author unagi ootoro
@help
插件修正:硕明云书
1、主要修复了不能正常缩放的问题及缩放数值设定
2、补全部分未定义参数
...
■ 改变角色图像的色调：
<tone: 255, 64, 64, 0>
例子：将色调设置为红色255、绿色64、蓝色64、灰度0
<tone: 255, 64, 64, 0>

■ 显示图片：
例子：显示名为Actor1_2的图片，并只显示图片的部分区域（X坐标100，Y坐标100，宽度200，高度200）
...
如果在第一页以外的页面上设置评论，则评论设置将优先。

■ 通过插件命令更改评论参数值
您可以通过插件命令更改评论指定的参数值。
有关详细信息，请参阅插件命令。
如果评论参数的值已更改，则该参数不会反映评论设置。

■ 角色旋转
“开始旋转”插件命令会旋转指定的角色。
执行“停止旋转”将停止角色的旋转。如果在此状态下再次执行“开始旋转”，旋转将恢复。
角色将恢复旋转。要返回到原始状态，请执行“停止旋转”然后“重置旋转”。恢复到原始状态。

[附录]
当你为插件参数/插件命令指定一个字符时，你可以输入一个变量值；
...
@text 图片名称
@type string
@desc 指定一个图像名称，该名称包含在插件参数“已注册图像列表”中。
...
(() => {
    ...
    character.setScaleX(scaleX);
    character.setScaleY(scaleY);
    });
